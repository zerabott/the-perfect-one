...
from comments import format_reply
from approval import admin_callback
from enhanced_reporting import show_report_reasons, handle_report_reason_callback, handle_cancel_report, handle_admin_dismiss_report, handle_submit_report
from admin_report_management import (
    handle_admin_delete_comment, handle_admin_delete_post,
    handle_confirm_delete_comment, handle_confirm_delete_post,
    handle_cancel_delete_comment, handle_cancel_delete_post
)
...
from admin_user_activity import admin_user_posts_callback, admin_user_comments_callback
...
async def handle_menu_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == 'writing_confession':
        ...
        return
    elif state == 'admin_writing_rejection_reason':
        # Handle admin's custom rejection reason input
        from approval import handle_admin_rejection_reason_input
        await handle_admin_rejection_reason_input(update, context)
        return
    elif state == 'admin_searching_user':
        # Handle admin user search input
        await handle_admin_user_search_input(update, context)
        return
...
async def handle_confession_submission(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.text:
        ...
        # Allow all content - no validation needed
        # Telegram's message limit (4096 characters) will be the natural boundary
...
async def handle_text_after_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Length validation removed - users can submit confessions of any length
    # Telegram's message limit (4096 characters) will be the natural boundary
...
async def show_comments_directly(update: Update, context: ContextTypes.DEFAULT_TYPE, post_id: int):
    """Show comments directly via deep link - uses same logic as see_comments_callback"""
    try:
    ...
    # Send each comment as a separate message with delay (same as see_comments_callback)
    for comment_index, comment_data in enumerate(comments_data):
        ...
        # Format comment text with proper line spacing for HTML parse mode
        from utils import format_date_only_html
        formatted_date = format_date_only_html(timestamp)
        
        # Check if this is a reply and format accordingly
        if is_reply and 'original_comment' in comment_data:
            original = comment_data['original_comment']
            original_content = original['content']
            
            # Truncate original content for display
            if len(original_content) > 150:
                original_preview = original_content[:150] + "..."
            else:
                original_preview = original_content
                
            # Get original comment sequential number
            original_seq_num = get_comment_sequential_number(original['comment_id'])
            
            # Use format_reply for consistent HTML blockquote styling
            from comments import format_reply
            reply_content = format_reply(original_preview, content)
        ...
        # Create reaction buttons - always show Reply button for all comments
        comment_keyboard = [
            [
        ...
        # Note: This is the show_comments_directly function which may have old unused code
        # The active function is see_comments_callback which properly handles flat comment structure
        if False:  # Disable this old nested reply logic
            for reply_index, nested_reply in enumerate(replies):
                ...
                if parent_comment_info:
                    # Create a quoted block exactly like Telegram forwarded message
                    ...
                    # Create the forwarded message style with a clickable link integrated into the quote header
                    # Use a deep link format that looks natural in the forwarded message style
                    bot_username = "@your_bot_username"  # You should replace this with your actual bot username
                    deep_link = f"https://t.me/{bot_username[1:]}?start=goto_comment_{parent_comment_id}"
                    
                    # Format exactly like Telegram forwarded messages with integrated clickable link
                    quoted_block = f"╭─ 💬 [Replying to comment\\# {parent_number}]({deep_link})\n┃ _{parent_preview}_\n╰────────────────────────────"
                    reply_text = f"{quoted_block}\n\n*reply\\# {sequential_reply_number}*\n\n{escape_markdown_text(reply_content)}\n\n{formatted_reply_date}"
                    
                    # Create reaction buttons for reply WITHOUT separate "View Original" button
                    reply_keyboard = [
                        [
                    ...
                    # Create reaction buttons for reply without navigation
                    reply_keyboard = [
                        [
                            InlineKeyboardButton(f"{reply_like_emoji} {reply_likes}", callback_data=f"like_comment_{reply_id}"),
                            InlineKeyboardButton(f"{reply_dislike_emoji} {reply_dislikes}", callback_data=f"dislike_comment_{reply_id}"),
        ...
        # This section is disabled because the flat comment structure doesn't use nested replies
...
async def handle_comment_submission(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Prepare content for saving and notification (may be formatted for replies)
    notification_content = content
    
    # If this is a reply, format it with the parent comment
    if reply_to_comment_id:
        try:
            from comments import get_parent_comment_for_reply, format_reply
            parent_info = get_parent_comment_for_reply(reply_to_comment_id)
            if parent_info:
                parent_text = parent_info['content']
                # Create formatted reply for notifications
                notification_content = format_reply(parent_text, content)
        except Exception as e:
    ...
    try:
        ...
        await notify_comment_on_post(context, post_id, notification_content, user_id, comment_id)
    ...
    # Message for normal comment vs. reply with formatted content
    if reply_to_comment_id:
        # Fetch parent comment so we can show it inside the blockquote
        parent_comment = get_comment_by_id(reply_to_comment_id)
        
        if parent_comment:
            formatted_text = format_reply(parent_comment[3], content)  # parent[3] = content
        else:
            formatted_text = content  # fallback if parent not found

        # Send formatted reply with HTML parse mode
        try:
            await update.message.reply_text(
                f"✅ Your reply was posted successfully!\n\n{formatted_text}",
                parse_mode="HTML"
            )
        except Exception as e:
            # Fallback to plain text if HTML fails
            logger.warning(f"HTML formatting failed for reply confirmation: {e}")
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Admin callbacks
    if data.startswith(("approve_", "reject_", "qreject_", "custom_reject_", "cancel_reject_", "flag_", "block_", "unblock_")):
    ...
    # Report comment - Enhanced reporting with immediate admin notification
    ...
    # Handle going back to report reasons
    if data.startswith("report_") and not data.startswith("report_reason_") and not data.startswith("report_comment_"):
        # This handles back to report selection: report_{content_type}_{content_id}
        parts = data.split('_')
        if len(parts) == 3:
            content_type = parts[1]
            content_id = int(parts[2])
            await show_report_reasons(update, context, content_type, content_id)
        return
    
    # Handle admin report dismissal
    if data.startswith("admin_dismiss_report_"):
        await handle_admin_dismiss_report(update, context)
        return
    
    # Handle admin user activity viewing
    ...
    # Handle admin delete actions from reports
    if data.startswith("admin_delete_comment_"):
        await handle_admin_delete_comment(update, context)
        return
        
    if data.startswith("admin_delete_post_"):
        await handle_admin_delete_post(update, context)
        return
    
    # Handle delete confirmations
    if data.startswith("confirm_delete_comment_"):
        await handle_confirm_delete_comment(update, context)
        return
        
    if data.startswith("confirm_delete_post_"):
        await handle_confirm_delete_post(update, context)
        return
    
    # Handle delete cancellations
    if data.startswith("cancel_delete_comment_"):
        await handle_cancel_delete_comment(update, context)
        return
    
    if data.startswith("cancel_delete_post_"):
        await handle_cancel_delete_post(update, context)
        return
    
    # Go to comment (navigation from quoted replies)
    if data.startswith("go_to_comment_"):
        target_comment_id = int(data.replace("go_to_comment_", ""))
        
        # Find which page the target comment is on
        from comments import find_comment_page
        navigation_info = find_comment_page(target_comment_id)
        
        if not navigation_info:
            await query.answer("❗ Comment not found or no longer available!")
            return
        
        post_id = navigation_info['post_id']
        page = navigation_info['page']
        
        # Show a brief feedback to the user
        await query.answer(f"🔗 Navigating to original comment...")
        
        # Navigate to the correct page with the target comment
        try:
            # Store navigation info to highlight the target comment if needed
            context.user_data['target_comment_id'] = target_comment_id
            
            # Delete the current message first
            await query.delete_message()
            
            # Navigate to the comments page containing the target comment
            # Create a fake update object to call see_comments_callback
            fake_query_data = f"see_comments_{post_id}_{page}"
            context.user_data['callback_data'] = fake_query_data
            context.user_data['refresh_comments'] = True
            
            # Call the see_comments_callback directly
            await see_comments_callback(update, context)
            
        except Exception as e:
            logger.error(f"Error navigating to comment {target_comment_id}: {e}")
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text="❗ Error navigating to the original comment. Please try again."
            )
    ...
    if data.startswith("admin_user_info_"):
        ...
        await admin_export_user_callback(update, context)
        return
...
"""
    ...
    if data == "contact_admin":
        ...
        # Create a fake update object to call the start_contact_admin function
        from telegram import Update as TgUpdate
        fake_update = TgUpdate(update_id=0, message=update.effective_message)
        fake_update._effective_chat = update.effective_chat
        fake_update._effective_user = update.effective_user
...
async def handle_admin_read_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        from admin_messaging import mark_message_as_read, update_admin_notifications
        ...
        if success:
            ...
            # Update all admin notifications to prevent duplicate actions
            await update_admin_notifications(context, message_id, user_id)
        else:
...
async def handle_admin_ignore_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        from admin_messaging import ignore_user_messages, update_admin_notifications, get_pending_messages
        ...
        if success:
            ...
            # Find message IDs from this user and update notifications for each
            pending_messages = get_pending_messages()
            for message_id, user_id_check, content, timestamp in pending_messages:
                if user_id_check == sender_user_id:
                    await update_admin_notifications(context, message_id, user_id)
        else:
...
async def admin_user_info_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show comprehensive detailed user information with activity stats and navigation"""
    query = update.callback_query
    await query.answer()
    ...
    try:
        with sqlite3.connect(DB_PATH) as conn:
            ...
            # Get basic user info
            cursor.execute("""
                SELECT user_id, username, first_name, last_name, join_date, questions_asked, comments_posted, blocked 
            ...
            cursor.execute("""
                SELECT 
                    ...
                    COUNT(CASE WHEN status = 'approved' OR approved = 1 THEN 1 END) as approved_posts,
                    COUNT(CASE WHEN status = 'rejected' OR approved = 0 THEN 1 END) as rejected_posts,
                    COUNT(CASE WHEN status IS NULL OR (approved IS NULL) THEN 1 END) as pending_posts,
                    COUNT(CASE WHEN flagged = 1 THEN 1 END) as flagged_posts
                FROM posts WHERE user_id = ?
            """, (target_user_id,))
            post_stats = cursor.fetchone()
            total_posts, approved_posts, rejected_posts, pending_posts, flagged_posts = post_stats or (0, 0, 0, 0, 0)
            
            # 2. Comment statistics  
            cursor.execute("""
                SELECT 
                    COUNT(*) as total_comments,
                    COUNT(CASE WHEN flagged = 1 THEN 1 END) as flagged_comments,
                    COUNT(CASE WHEN parent_comment_id IS NOT NULL THEN 1 END) as replies_made
                FROM comments WHERE user_id = ?
            """, (target_user_id,))
            comment_stats = cursor.fetchone()
            total_comments, flagged_comments, replies_made = comment_stats or (0, 0, 0)
            
            # 3. Reaction statistics (given and received)
            # Reactions given by user
            cursor.execute("""
                SELECT COUNT(*) FROM reactions WHERE user_id = ? AND target_type = 'comment'
            """, (target_user_id,))
            reactions_given = cursor.fetchone()[0]
            
            # Reactions received on user's comments
            cursor.execute("""
                SELECT 
                    COUNT(CASE WHEN reaction_type = 'like' THEN 1 END) as likes_received,
                    COUNT(CASE WHEN reaction_type = 'dislike' THEN 1 END) as dislikes_received
                FROM reactions r 
                JOIN comments c ON r.target_id = c.comment_id AND r.target_type = 'comment'
                WHERE c.user_id = ?
            """, (target_user_id,))
            reaction_stats = cursor.fetchone()
            likes_received, dislikes_received = reaction_stats or (0, 0)
            ...
            # 5. Latest activity samples
            cursor.execute("""
                SELECT 'post' as type, post_id as id, content, category, timestamp, 
                       CASE WHEN approved = 1 THEN 'approved' 
                            WHEN approved = 0 THEN 'rejected' 
                            ELSE 'pending' END as status
                FROM posts WHERE user_id = ?
                UNION ALL
                SELECT 'comment' as type, comment_id as id, content, 
                       (SELECT category FROM posts WHERE post_id = comments.post_id) as category, 
                       timestamp, 'active' as status
                FROM comments WHERE user_id = ?
                ORDER BY timestamp DESC LIMIT 3
            """, (target_user_id, target_user_id))
            recent_activity = cursor.fetchall()
            
        # Format the comprehensive user info
        user_text = f"""👤 *Detailed User Information*
...
*📊 Content Statistics:*
• Total Posts: {total_posts} \\({approved_posts} approved, {rejected_posts} rejected, {pending_posts} pending\\)
• Total Comments: {total_comments} \\({replies_made} replies to others\\)
• Flagged Content: {flagged_posts} posts, {flagged_comments} comments

*👍 Engagement:*
• Reactions Given: {reactions_given}
• Likes Received: {likes_received}
• Dislikes Received: {dislikes_received}
• Engagement Ratio: {f"{likes_received/(likes_received + dislikes_received)*100:.1f}%" if (likes_received + dislikes_received) > 0 else "No reactions"}

*📅 Recent Activity \\(7 days\\):*
• Posts: {recent_posts} | Comments: {recent_comments}
"""
        
        # Add recent activity sample if available
        if recent_activity:
            user_text += "\n*🕐 Latest Activity:*\n"
            for activity in recent_activity[:3]:
                activity_type, activity_id, content, category, timestamp, status = activity
                # Truncate content for preview
                content_preview = (content[:40] + "...") if content and len(content) > 40 else (content or "[No content]")
                try:
                    from datetime import datetime
                    dt = datetime.fromisoformat(timestamp.replace('Z', '+00:00'))
                    time_ago = format_time_ago(dt)
                ...
                emoji = "📝" if activity_type == "post" else "💬"
                status_emoji = "✅" if status == "approved" else "❌" if status == "rejected" else "⏳" if status == "pending" else "💬"
                user_text += f"• {emoji} {escape_markdown_text(activity_type.title())} \\#{activity_id} {status_emoji} \\- {escape_markdown_text(content_preview)} \\({escape_markdown_text(time_ago)}\\)\n"
        
        # Create comprehensive navigation buttons
        keyboard = [
            [
                InlineKeyboardButton("📝 View All Posts", callback_data=f"admin_user_posts_{uid}"),
                InlineKeyboardButton("💬 View All Comments", callback_data=f"admin_user_comments_{uid}")
            ],
            [
                InlineKeyboardButton("📊 Export User Data", callback_data=f"admin_export_user_{uid}"),
        ...
        logger.error(f"Error in admin_user_info_callback: {e}")
        await query.answer(f"❗ Error loading user info: {str(e)}")
        try:
            await query.edit_message_text(
...
• First/Last name \\(e\\.g\\. `John` or `John Doe`\\)

*Examples:*
• `123456789` \\- Search by exact user ID
• `john` \\- Search by name containing "john"
...
async def handle_admin_user_search_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle admin user search input and perform search"""
    user_id = update.message.from_user.id
    ...
    if not search_query:
        await update.message.reply_text(
            "❗ Please enter a valid search query (User ID, username, or name)."
        )
        return
    ...
    try:
        # Import search manager if available, or implement basic search
        try:
            from search_manager import search_users
            search_results = search_users(search_query)
        except ImportError:
            # Fallback: implement basic search directly
            search_results = perform_basic_user_search(search_query)
        
        if not search_results:
            no_results_text = f"""🔍 *Search Results*

❌ **No users found for:** `{escape_markdown_text(search_query)}`
...
Use /admin to return to admin dashboard."""
        ...
        await display_user_search_results(update, context, search_results, search_query)
        
    except Exception as e:
        logger.error(f"Error in handle_admin_user_search_input: {e}")
        await update.message.reply_text(
            f"❗ Error performing search: {str(e)}\n\nPlease try again or use /admin to return to dashboard."
        )
        context.user_data.pop('state', None)

def perform_basic_user_search(query):
    """Perform basic user search when search_manager is not available"""
    with sqlite3.connect(DB_PATH) as conn:
        cursor = conn.cursor()
        
        results = []
        
        # Try exact user ID match first
        if query.isdigit():
            cursor.execute(
                "SELECT user_id, username, first_name, last_name, join_date, questions_asked, comments_posted, blocked FROM users WHERE user_id = ?",
                (int(query),)
            ...
            if exact_match:
                results.append({
                    ...
                    'username': exact_match[1],
                    'first_name': exact_match[2],
                    'last_name': exact_match[3],
                    'join_date': exact_match[4],
                    'questions_asked': exact_match[5],
                    'comments_posted': exact_match[6],
                    'blocked': exact_match[7],
                    'match_type': 'exact_id'
                })
                return results
        
        # Clean query for text searches
        clean_query = query.lstrip('@').lower()
        
        # Search by username (exact match)
        cursor.execute(
            "SELECT user_id, username, first_name, last_name, join_date, questions_asked, comments_posted, blocked FROM users WHERE LOWER(username) = ? LIMIT 5",
            (clean_query,)
        )
        username_matches = cursor.fetchall()
        for match in username_matches:
            results.append({
                ...
                'last_name': match[3],
                'join_date': match[4],
                'questions_asked': match[5],
                'comments_posted': match[6],
                'blocked': match[7],
                'match_type': 'username'
            })
        ...
        if len(results) < 5:
            cursor.execute(
                ...
                LIMIT ?""".format(','.join(str(r['user_id']) for r in results) if results else '0'),
                (f'%{clean_query}%', f'%{clean_query}%', f'%{clean_query}%', 5 - len(results))
            )
...
async def display_user_search_results(update: Update, context: ContextTypes.DEFAULT_TYPE, search_results, search_query):
...
**Query:** `{escape_markdown_text(search_query)}`
**Found:** {results_count} user{'s' if results_count != 1 else ''}
...
👤 User profiles with admin controls:"""
    ...
    for i, user in enumerate(search_results, 1):
        ...
        join_date = user.get('join_date') or 'Unknown'
        questions_asked = user.get('questions_asked', 0)
        comments_posted = user.get('comments_posted', 0)
        blocked = user.get('blocked', False)
        match_type = user.get('match_type', 'unknown')
        
        # Format display name
        display_name = f"{first_name} {last_name}".strip() or username or "Anonymous"
        
        # Format join date
        try:
            if join_date and join_date != 'Unknown':
                from datetime import datetime
                if 'T' in join_date or ' ' in join_date:
                    dt = datetime.fromisoformat(join_date.replace('Z', '+00:00'))
                    formatted_date = dt.strftime('%Y-%m-%d')
                else:
                    formatted_date = join_date[:10]
            else:
        ...
        user_text = f"""**{i}\\. {match_emoji} {escape_markdown_text(display_name)}**

*Details:*
• ID: `{user_id}`
• Username: @{escape_markdown_text(username)} 
• Status: {status_emoji} {escape_markdown_text(status_text)}
• Joined: {escape_markdown_text(formatted_date)}

*Activity:*
• Posts: {questions_asked}
• Comments: {comments_posted} 
• Total Activity: {total_activity}
• Match Type: {escape_markdown_text(match_type.replace('_', ' ').title())}"""
        ...
        if blocked:
            keyboard.append([
                InlineKeyboardButton("✅ Unblock User", callback_data=f"admin_unblock_{user_id}"),
                InlineKeyboardButton("👤 Detailed Info", callback_data=f"admin_user_info_{user_id}")
            ])
        else:
            keyboard.append([
        ...
        # Add view posts/comments buttons if user has activity
        if questions_asked > 0 or comments_posted > 0:
            activity_buttons = []
            if questions_asked > 0:
                activity_buttons.append(
                    InlineKeyboardButton(f"📝 Posts ({questions_asked})", callback_data=f"admin_user_posts_{user_id}")
                )
            if comments_posted > 0:
                activity_buttons.append(
                    InlineKeyboardButton(f"💬 Comments ({comments_posted})", callback_data=f"admin_user_comments_{user_id}")
                )
            if activity_buttons:
                keyboard.append(activity_buttons)
        ...
        # Small delay between user profiles
        await asyncio.sleep(0.3)
...
async def handle_admin_delete_post_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Show confirmation dialog - properly handle content preview
    if len(post_details['content']) > 100:
        content_preview = escape_markdown_text(post_details['content'][:100]) + "\\.\\.\\."
    else:
        content_preview = escape_markdown_text(post_details['content'])
...
async def handle_admin_delete_comment_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Show confirmation dialog - properly handle content preview
    if len(comment_details['content']) > 100:
        content_preview = escape_markdown_text(comment_details['content'][:100]) + "\\.\\.\\."
    else:
        content_preview = escape_markdown_text(comment_details['content'])
...
async def admin_export_user_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle admin export user data callback"""
    query = update.callback_query
    await query.answer()
    ...
    target_user_id = int(query.data.replace("admin_export_user_", ""))
    
    try:
        # Show processing message
        await query.edit_message_text(
            "📊 *Exporting User Data*\n\n⏳ Please wait while compiling comprehensive user data...",
            parse_mode="MarkdownV2"
        )
        
        # Try to use export manager if available
        try:
            from admin_tools import export_manager
            success, result = export_manager.export_user_data(target_user_id)
            
            if success:
                success_text = f"""✅ *User Data Export Complete*

📁 **Export Details:**
• User ID: `{target_user_id}`
• Export File: `{escape_markdown_text(result)}`
• Format: JSON (GDPR compliant)
• Content: All user posts, comments, reactions, and admin messages

🗂️ The export file has been saved to the exports directory and contains all data associated with this user account."""
            else:
                success_text = f"""❌ *Export Failed*
...
Please check the system logs and try again."""
            ...
            # Fallback: basic manual export using database queries
            success, result = await export_user_data_basic(target_user_id)
...
📁 **Export Summary:**
• User ID: `{target_user_id}`
• Posts: {result.get('posts', 0)} confessions
• Comments: {result.get('comments', 0)} comments
• Reactions: {result.get('reactions', 0)} reactions given
• Messages: {result.get('admin_messages', 0)} admin messages

📊 Export data compiled successfully."""
            else:
                success_text = f"""❌ *Export Failed*

🚨 Error during basic export:
`{escape_markdown_text(str(result))}`
...
Please try again or contact system administrator."""
        ...
        keyboard = [
            [
                InlineKeyboardButton("👤 View User Info", callback_data=f"admin_user_info_{target_user_id}"),
                InlineKeyboardButton("📋 User Management", callback_data="admin_users")
            ],
            [
                InlineKeyboardButton("🔙 Back to Dashboard", callback_data="admin_dashboard")
        ...
        logger.error(f"Error in admin_export_user_callback: {e}")
        await query.edit_message_text(
            "❌ *Error*\n\nFailed to export user data\\. Please try again\\.",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton(f"👤 Back to User Info", callback_data=f"admin_user_info_{target_user_id}")
            ]]),
            parse_mode="MarkdownV2"
        )

async def export_user_data_basic(target_user_id: int):
    """Basic user data export when export_manager is not available"""
    try:
        with sqlite3.connect(DB_PATH) as conn:
            cursor = conn.cursor()
            ...
            # Count comments
            cursor.execute("SELECT COUNT(*) FROM comments WHERE user_id = ?", (target_user_id,))
            comments_count = cursor.fetchone()[0]
            
            # Count reactions given
            cursor.execute("SELECT COUNT(*) FROM comment_reactions WHERE user_id = ?", (target_user_id,))
            reactions_count = cursor.fetchone()[0]
            
            # Count admin messages
            cursor.execute("SELECT COUNT(*) FROM admin_messages WHERE user_id = ?", (target_user_id,))
            admin_messages_count = cursor.fetchone()[0]
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(enhanced_ranking_callback_handler, pattern=r"^(enhanced_ranking|leaderboard_|ranking_|achievement_view_my|rank_ladder|rank_point_guide)"))
