...
def sanitize_content(text):
    """Sanitize content - allows all types of messages"""
    if not text:
        return text  # Allow empty text for media-only posts
    
    # Only remove excessive whitespace, but allow all content
    text = re.sub(r'\s+', ' ', text).strip()
    
    # Return the text as-is, no validation or spam checking
    return text
...
def format_date_only_html(timestamp_str):
    """Format timestamp to show only date part with proper HTML escaping"""
    try:
        from datetime import datetime
        dt = datetime.fromisoformat(timestamp_str.replace('Z', '+00:00'))
        formatted_date = dt.strftime('%Y-%m-%d')
        # Escape HTML characters in the formatted date
        return escape_html_text(formatted_date)
    except:
        return escape_html_text(str(timestamp_str)[:10])  # Fallback to first 10 chars
