...
"""
    ...
    for admin_id in ADMIN_IDS:
        try:
            ...
            save_admin_notification(message_id, admin_id, sent_message.message_id)
            success_count += 1
...
""",
        ...
        # Update all admin notifications to prevent duplicate replies
        await update_admin_notifications(context, message_id, admin_id)
...
def save_admin_notification(message_id, admin_id, telegram_message_id):
    """Save admin notification record"""
    try:
        with sqlite3.connect(DB_PATH) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO admin_notifications (message_id, admin_id, telegram_message_id) VALUES (?, ?, ?)",
                (message_id, admin_id, telegram_message_id)
            )
            conn.commit()
            return True
    except Exception as e:
        logger.error(f"Error saving admin notification: {e}")
        return False

def get_active_admin_notifications(message_id):
    """Get all active admin notifications for a message"""
    try:
        with sqlite3.connect(DB_PATH) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT admin_id, telegram_message_id FROM admin_notifications WHERE message_id = ? AND status = 'active'",
                (message_id,)
            )
            return cursor.fetchall()
    except Exception as e:
...
def mark_admin_notifications_handled(message_id):
    """Mark all admin notifications for a message as handled"""
    try:
        with sqlite3.connect(DB_PATH) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE admin_notifications SET status = 'handled' WHERE message_id = ?",
                (message_id,)
            )
            conn.commit()
            return cursor.rowcount > 0
    except Exception as e:
        logger.error(f"Error marking notifications as handled: {e}")
        return False

async def update_admin_notifications(context, message_id, admin_who_replied):
    """Update all admin notifications when one admin takes action"""
    from telegram import InlineKeyboardMarkup, InlineKeyboardButton
    
    # Get all active notifications for this message
    notifications = get_active_admin_notifications(message_id)
    
    # Mark all notifications as handled
    mark_admin_notifications_handled(message_id)
    
    # Create disabled keyboard
    keyboard = [[
        InlineKeyboardButton("✅ Already Handled", callback_data="handled")
    ]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    # Update each admin's message
    for admin_id, telegram_message_id in notifications:
        if admin_id != admin_who_replied:  # Don't update the admin who replied
            try:
...
This message has already been replied to by another admin\\.
No further action needed\\.
""",
                    reply_markup=reply_markup,
                    parse_mode="MarkdownV2"
                )
                logger.info(f"Updated notification for admin {admin_id}")
            except Exception as e:
                logger.error(f"Failed to update notification for admin {admin_id}: {e}")
